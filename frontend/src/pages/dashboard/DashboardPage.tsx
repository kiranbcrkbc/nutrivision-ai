...
export const DashboardPage: React.FC = () => {
  ...
  return <div className="space-y-7 max-w-6xl mx-auto">
    <PageHeader title={`Welcome, ${user?.fullName?.split(' ')[0] || 'there'}.`} subtitle="A little space to focus on your wellbeing." />
    <section className="dashboard-welcome"><div><p className="eyebrow">YOUR PERSONAL WELLBEING JOURNAL</p><h2>Start with a small step.</h2><p>Record a concern, check your photo, and keep your observations together for your next conversation with a clinician.</p><Link to="/assessment/new" className="wellness-button">New photo check <ArrowUpRight size={18}/></Link></div><Leaf aria-hidden="true" size={120} strokeWidth={.7}/></section>
    ...
    <div className="grid sm:grid-cols-2 gap-4">{[['Food guidance','Everyday food sources, tailored to your dietary preferences.','/recommendations'],['Your activity','A simple view of the records you have saved.','/progress'],['Your reports','Keep a copy for your next consultation.','/reports'],['Nutrition questions','Explore common questions about food and vitamins.','/chatbot']].map(([title,text,url])=><Link key={url} to={url}><Card className="p-6 h-full hover:border-health-500"><div className="flex justify-between gap-4"><h2 className="font-semibold">{title}</h2><ArrowUpRight size={18} className="text-health-600"/></div><p className="mt-2 text-sm text-slate-500">{text}</p></Card></Link>)}</div>
  </div>;
};
