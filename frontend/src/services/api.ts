...
export const AI_SERVICE_URL = import.meta.env.VITE_AI_SERVICE_URL || 'http://localhost:8000/api/ai';
...
api.interceptors.response.use(
  ...
  (error: AxiosError<{ message?: string; error?: { message?: string } }>) => {
    if (!error.response) {
      ...
      return Promise.reject(new Error('Unable to connect to NutriVision AI Backend. Please verify that the server is running.'));
    }
